...
const DOG_ACTIONS: { anim: DogAnim; speech: string; speechEmoji: string }[] = [
  { anim: 'bark',  speech: 'WOOF WOOF!!',          speechEmoji: '🗣️' },
  { anim: 'bark',  speech: 'ARF ARF!! 🎉',          speechEmoji: '🐾' },
  { anim: 'lick',  speech: '*lick lick lick!*',     speechEmoji: '😛' },
  { anim: 'lick',  speech: 'Mmm you taste great!',  speechEmoji: '😋' },
  { anim: 'wag',   speech: 'SO HAPPY!! 🐾',         speechEmoji: '🥳' },
  { anim: 'wag',   speech: 'PET ME PET ME! 🥺',     speechEmoji: '❤️' },
  { anim: 'spin',  speech: '*spinning with joy!*',  speechEmoji: '🌀' },
  { anim: 'pant',  speech: 'SQUIRREL!! 🐿️',        speechEmoji: '👀' },
  { anim: 'pant',  speech: 'TREAT? TREAT? 🦴',      speechEmoji: '🍖' },
  { anim: 'bark',  speech: "YOU'RE THE BEST! ⭐",    speechEmoji: '⭐' },
  { anim: 'wag',   speech: 'WANNA GO WALKIES? 🌳',  speechEmoji: '🦮' },
  { anim: 'bark',  speech: 'MISSION: BELLY RUBS!',  speechEmoji: '🎯' },
]
...
async function playSound(type: 'coin' | 'lucky' | 'bark' | 'spin' | 'diamond') {
  try {
    ...
    if (type === 'coin') {
      for (const [i, freq] of ([523, 784] as const).entries()) {
      ...
      for (const [i, freq] of ([523, 659, 784, 1047] as const).entries()) {
        const o = ctx.createOscillator(); const g = ctx.createGain()
        o.connect(g); g.connect(ctx.destination)
        o.frequency.value = freq; o.type = 'sine'
      ...
      for (const [i, freq] of ([784, 988, 1319, 1760] as const).entries()) {
        const o = ctx.createOscillator(); const g = ctx.createGain()
        o.connect(g); g.connect(ctx.destination)
        o.frequency.value = freq; o.type = 'sine'
...
// ── Dog speech bubble (floating, prominent) ───────────────────────────────────
function DogSpeechBubble({ speech, speechEmoji }: { speech: string; speechEmoji: string }) {
  return (
    <div className="absolute inset-x-0 top-[72px] z-40 pointer-events-none flex justify-start px-3">
      <div className="bounce-in bg-white rounded-3xl px-5 py-3 shadow-2xl border-3 border-gray-800 max-w-[75vw] relative"
        style={{ borderWidth: 3, borderColor: '#1f2937' }}>
        <div className="flex items-center gap-2">
          <span className="text-3xl">{speechEmoji}</span>
          <p className="text-xl font-black text-gray-900">{speech}</p>
        </div>
        {/* Tail pointing down-left toward dog */}
        <div className="absolute -bottom-[13px] left-5">
          <div className="w-0 h-0"
            style={{ borderLeft: '10px solid transparent', borderRight: '10px solid transparent', borderTop: '13px solid #1f2937' }} />
          <div className="absolute w-0 h-0"
            style={{ top: -14, left: -8, borderLeft: '8px solid transparent', borderRight: '8px solid transparent', borderTop: '12px solid white' }} />
        </div>
      </div>
    </div>
  )
...
function DogStrip({
  ...
  diamonds: number
  onGiveDiamond: () => void
  onDogAction: (speech: string, speechEmoji: string, anim: DogAnim) => void
}) {
  ...
  const handleDogTap = useCallback(() => {
    ...
    // Sound
    if (pick.anim === 'spin') playSound('spin')
    else                      playSound('bark')

    // Animate
    if (animTimer.current) clearTimeout(animTimer.current)
    setDogAnim(pick.anim)
    const dur = pick.anim === 'spin' ? 700 : pick.anim === 'lick' ? 800 : 600
    animTimer.current = setTimeout(() => setDogAnim('idle'), dur)

    // Notify parent to show floating speech
    onDogAction(pick.speech, pick.speechEmoji, pick.anim)
  }, [onDogAction])
  ...
  return (
    <div className="flex-shrink-0 flex items-center gap-2 rounded-2xl px-3 py-2 shadow border-2"
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] font-semibold text-gray-500">tap Shizu!</p>
      </div>

      {/* Coins */}
      <div className="flex items-center gap-1 px-2 py-1 rounded-xl bg-white/60 border border-amber-200">
        <span className="text-base">🪙</span>
        <span className="text-sm font-black text-amber-800">{totalCoins}</span>
      </div>

      {/* Diamonds + give button */}
      <div className="flex items-center gap-1 px-2 py-1 rounded-xl bg-white/60 border border-cyan-200">
        <span className="text-base">💎</span>
        <span className="text-sm font-black text-cyan-700">{diamonds}</span>
        <button
          onClick={onGiveDiamond}
          className="ml-1 w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black text-cyan-700 bg-cyan-100 border border-cyan-400 diamond-pulse select-none">
          +
        </button>
      </div>
    </div>
...
export default function KidTracker({
  ...
  const [dogSpeech, setDogSpeech]         = useState<{ speech: string; speechEmoji: string } | null>(null)
  ...
  // Detect diamond awarded → float-up + chime
  useEffect(() => {
  ...
  const showReaction = useCallback((lucky: boolean, coinAmount: number) => {
    if (lucky) {
      setReaction({ text: LUCKY_MESSAGES[Math.floor(Math.random() * LUCKY_MESSAGES.length)], emoji: '🍀', coins: coinAmount })
      playSound('lucky')
    } else {
      setReaction({
        text:  FUN_MESSAGES[Math.floor(Math.random() * FUN_MESSAGES.length)],
  ...
  const handleDogAction = useCallback((speech: string, speechEmoji: string, _anim: DogAnim) => {
    setDogSpeech({ speech, speechEmoji })
    if (dogSpeechTimer.current) clearTimeout(dogSpeechTimer.current)
    dogSpeechTimer.current = setTimeout(() => setDogSpeech(null), 2200)
  }, [])

  // Detect 100% → mystery box check
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full gap-2 relative">
      ...
      {/* Task completion reaction */}
      {reaction && <FloatingReaction text={reaction.text} emoji={reaction.emoji} coins={reaction.coins} />}

      {/* Dog speech bubble (prominent floating) */}
      {dogSpeech && <DogSpeechBubble speech={dogSpeech.speech} speechEmoji={dogSpeech.speechEmoji} />}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto panel-scroll flex flex-col gap-2.5 pb-2">
        {pendingActivities.map((a, i) => (
          <TaskCard key={a.id} activity={a} index={i} onTap={async () => {
            ...
            const earned = lucky ? 3 : 2
            showReaction(lucky, earned)
            onCoinsNeedRefresh() // refresh coin total in parent immediately
          }} />
        ))}
